var userJqGrid = {
	...
	init:function(){ //初始化Grid数据
	    jQuery("#userQueryTable").jqGrid({
	        ...
	        width:'100%',
	        height:'100%',
	        colNames:['权限','ID','账号','编号','姓名', '邮箱', '联系方式','岗位','部门名称', '状态', '用户类型', '创建时间', '创建人', '备注'],
			colModel:[
				...
				{name:'dep.name',index:'depName',width:80, editable:true, editoptions:{size:"25",maxlength:"30"},editrules:{required:true} ,formoptions:{elmsuffix:"<span style=\"color:red\">*</span>"}}, 
	    	...
	    	{},
	    	{}
	    ).navButtonAdd('#userQueryPager',{   
	    	   caption:"批量分配",  
	    	   title:"批量分配",
	    	   buttonicon:"icon-pencil blue",    
	    	   onClickButton: function(){    
	    		   var rowid = $("#userQueryTable").jqGrid("getGridParam", "selarrrow"); //多行  
	    		   var rowids = [];
		    		for (var i in rowid) {
		    				rowids.push(rowid[i]);
		    		}
		    		if (rowids.length > 0) {
		    			...
		    			jQuery.ajax({
		    				type: "POST",
		    				url: basePath + "/userRole/queryUserRoleList",
		    				dataType : 'json',
		    				data: "userIds=" + rowids,
		    				success: function(data){
		    					if(data!=null && data.code==30000){
		    						alert("选择的用户，已经分配过权限，请重新选择");
		    						return;
		    						
		    					}else{
		    						showUserRoles(rowids);
		    					}
		    				},
		    				error : function(data) {
		    					alert("获取可用角色异常");
...
/**批量分配角色给用户**/
function showUserRoles(userIds){
	$("#userIds").attr("value",'');
	$("#userIds").val(userIds);
	$("#select2").empty();
	$("#role_user_info").html("");
	$("#role_user_id").attr("value",'');// 清空内容
	jQuery.ajax({
		type: "POST",
	...
	$('#user-role-modal-form').modal('show');
}
